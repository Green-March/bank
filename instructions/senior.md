...
workflow:
  ...
  - step: 8.1
    action: reviewer_timeout_fallback
    trigger: "reviewer_timeout_seconds (120秒) 経過後も応答 YAML が null または acknowledgement-only"
    flow:
  ...
  - step: 15
    ...
    from: junior
    via: send-keys
    note: "Junior からの成果物完了通知"
  - step: 16
    action: read_report
    target: "queue/reports/junior{N}_report.yaml"
  - step: 17
    action: route_review_request
    note: "request_type: deliverable_review を確認し、queue/review/junior_to_reviewer.yaml に中継する"
  - step: 18
    action: send_keys
    target: reviewer_pane_id_from_lookup
    method: two_bash_calls
  ...
  - step: 21
    action: read_yaml
    target: "queue/review/reviewer_to_junior.yaml"
  - step: 22
    action: handle_review_result
    note: "deliverable_review は revise のみ Junior に中継し、ok は step 23 に進む。"
  ...
  - step: 24
    ...
    followup:
      ...
      - "Enter"
    util_command: "templates/senior_clear_junior.sh <junior_pane_id> \"instructions/juniorN.md を読んで役割を理解してください。\" <junior_number>"
  - step: 25
...
## Reviewer タイムアウト自動フォールバック（必須）
Reviewer 通知後、`reviewer_timeout_seconds`（120秒）以内に応答 YAML が書き込まれない場合、以下のフォールバックを実行する。
これは Phase B（計画レビュー、step 8）と Phase D（成果物レビュー、step 19）の両方に適用される。

### フォールバックフロー
1. Reviewer に send-keys 通知後、120秒経過したら応答 YAML を1回読む。
2. 応答が null または acknowledgement-only の場合 → corrective message を送信する。
   - メッセージに含める内容: `verdict/comments/suggested_changes` の出力形式、`./templates/reviewer_finalize.sh` の使用指示、簡潔な出力の要請。
...
標準実装コマンド:
```bash
templates/senior_clear_junior.sh <junior_pane_id> "instructions/juniorN.md を読んで役割を理解してください。" <junior_number>
```
